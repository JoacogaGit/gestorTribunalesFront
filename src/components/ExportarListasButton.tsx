...
export default function ExportarListasButton({ vocaliaId, nombreOficina, esEstudio, compact }: Props) {
  ...
  if (compact) {
    return (
      <button
        ...
        className="flex h-8 w-8 items-center justify-center rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors disabled:opacity-50"
      >
  ...
  return (
    <Button size="sm" variant="outline" onClick={handleClick} disabled={!vocaliaId || loading}>
