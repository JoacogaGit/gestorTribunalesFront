...
DESPACHANTE: si el Excel tiene una columna tipo DESPACHANTE, DESP, RESPONSABLE o similar, extraer iniciales del nombre (primera letra de cada palabra). Ej: PATRICIO GASTÓN FLORES → PGF. Si ya tiene 3 caracteres o menos, usar tal cual. Guardar en el campo despachante de la causa.
CATEGORÍAS: si el Excel tiene columnas con un patrón claramente reconocible de categoría con valores estructurados (ej: columna PRUEBA PROVEIDA con valores si/no, columna INSTRUCCION SUPLEMENTARIA con valores cumplida/pendiente, columna CITADO con valores si/no), incluirlas como eventos sin fecha: titulo = nombre de la columna, descripcion = valor de la celda, fecha_hora = null, tipo_evento = categoria.`;
